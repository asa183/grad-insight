...
function detectSite(url, site) {
    ...
    if (/edu\.hokudai\.ac\.jp\/graduate_school\/department\/academic/.test(url))
        return 'edu';
    if (/faculty-member/.test(url))
        return 'fish';
    return 'other';
}
async function main() {
    if (!SHEET_ID) {
        console.error('SHEET_ID required');
        process.exit(2);
    }
    if (!GOOGLE_CREDENTIALS_JSON) {
        console.error('GOOGLE_CREDENTIALS_JSON required');
        process.exit(2);
    }
    ...
    const header = rows[0];
    const getIdx = (names) => names.map(n => header.findIndex(h => String(h).trim().toLowerCase() === n)).find(i => i >= 0) ?? -1;
    const urlIdx = (() => {
    ...
    if (missing.length) {
        console.error('Required URLs missing in Examples:', missing.join(', '));
        process.exit(1);
    }
