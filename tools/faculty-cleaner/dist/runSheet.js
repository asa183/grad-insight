...
async function fetchHtml(url, timeoutMs = 15000) {
    ...
    try {
        const res = await fetch(url, { redirect: 'follow', headers: { 'user-agent': 'grad-insight-bot/1.0' }, signal: ctrl.signal });
        if (!res.ok)
...
async function ensureAnyoneReader(drive, fileId) {
    try {
        ...
        if (e?.response?.status !== 400)
            console.warn('permission warn:', e?.message || e);
    }
}
...
async function main() {
    ...
    async function worker(wid) {
        while (true) {
            ...
            if (!t)
                return;
            const r1 = t.rowIndex + 1;
            const url = t.values[COL.URL];
            const existingId = t.values[COL.FILE_ID];
            const name = slugifyUrl(url);
            try {
